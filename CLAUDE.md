...
CI (`.github/workflows/build.yml`) runs `./gradlew build` on every push/PR — `build` runs the tests as well as assembling. Releases are tag-driven (`v*`) via `release.yml`, which also runs `./gradlew build` so releases are gated on passing tests; publishing to Maven Central is manual via the **Central Portal** (OSSRH was shut down 2025-06-30): `./gradlew publish` uploads through the OSSRH Staging API and then (for releases) runs `promoteStagingRepository` as a `finalizedBy` step — an authenticated `POST .../manual/upload/defaultRepository/<namespace>?publishing_type=user_managed` that moves the staging repo into the Portal so the deployment is actually visible under Deployments; without it the upload never shows up. You then finalize (verify + Publish) at central.sonatype.com. Snapshots publish directly and skip the promote. Credentials are a Central Portal **user token** in `local.properties` (`ossrh.user`/`ossrh.key`) — see the comments at the bottom of `build.gradle`.
...
Each record type parses its own RDATA. `AbstractRecord.parseData(short dataLength, Buffer)` is the contract; ~30 concrete classes read fields off the `Buffer` and implement `toString()` for `dig`-style output. These span the common types (`ARecord`, `AAAARecord`, `CNAMERecord`, `MXRecord`, `NSRecord`, `PTRRecord`, `SOARecord`, `SPFRecord`, `TXTRecord`, `CAARecord`, `SRVRecord`, `NAPTRRecord`, `HINFORecord`, `KXRecord`, `DNAMERecord`, `URIRecord`, `EUI48Record`, `EUI64Record`), the DNSSEC family (`DSRecord`, `DNSKEYRecord`, `RRSIGRecord`, `NSECRecord`, `NSEC3Record`, `CDSRecord`, `CDNSKEYRecord`, `SSHFPRecord`, `TLSARecord`, `SMIMEARecord`), and the service-binding family (`SVCBRecord`, with `HTTPSRecord extends SVCBRecord` since the two share a wire format and differ only in display name). The authoritative list of parsed types is the `RecordType` enum's class-level javadoc. `UnknownRecord` is the fallback that just captures raw bytes.
